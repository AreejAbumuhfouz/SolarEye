...
import OTPVerification from "../Pages/OTPVerification"; // OTP Modal
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Send user data to the server to sign up
      const response = await axios.post('http://localhost:5000/api/auth/signup', formData);
      
      // If signup is successful, open the OTP modal
  ...
  return (
    ...
    >
      ...
      {/* OTP Modal (popup) */}
      {showOTPModal && <OTPVerification onClose={() => setShowOTPModal(false)} />}
    </div>
  );
};
